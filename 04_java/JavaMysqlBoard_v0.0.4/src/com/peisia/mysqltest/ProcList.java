...
public class ProcList {
	static public final int PER_PAGE = 6;
...
//			select count(*) from board where b_reply_ori is null;
			String sql = "select * from board where b_reply_orino is null limit "+startIndex+","+PER_PAGE;
			try {
				Cw.wn("전송한sql문:"+sql);
				...
				while(Db.result.next()) {	// 결과를 하나씩 빼기. 더 이상 없으면(행 수가 끝나면) false 리턴됨.
					...
					String datetime = Db.result.getString("b_time");
