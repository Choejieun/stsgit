...
maintextarea.innerHTML +="전투시작\n";

while (true) {

	function getRandomAttackValue(attack) {
		attack = attack + 1;    //ex. 공격력이 10이라고 하면 0~10의 수치로 나와야 하므로
		var random = Math.floor(Math.random() * attack)+50;  // ex. 공격력이 11이고 이 랜덤처리를 하면 랜덤 값은 0~10이 나옴
		return random;
	}
	...
	maintextarea.innerHTML += "💠 " + elf.name + "는 " + orc.name + "에게 💥일반공격을 시도하였습니다."+'\n';
	maintextarea.innerHTML += orc.name + "는 " + elfdamage + "의 피해를 입었습니다."+'\n';
	maintextarea.innerHTML += orc.info() + elf.info() + '\n';
	 ...
	 if (elf.hp < 1) {
		...
		function getRandomgoldgivealue(goldgive) {
			goldgive = goldgive +1;   
			var random = Math.floor(Math.random() * goldgive);  
			return random;
		}

		var goldup = getRandomAttackValue(orc.goldgive);
		maintextarea.value += maintextarea + elf.win()+'\n'+'\n'+"획득한 경험치 : " + expup+'\n'+"획득한 골드 : " + goldup+'\n\n'; // 승리 리액션 출력
		break;
	}
	...
	maintextarea.innerHTML += elf.name + "는 " + orcdamage+ "의 피해를 입었습니다." + '\n';
	maintextarea.innerHTML += '\n' + orc.info() + elf.info();
