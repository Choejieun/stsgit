<%@page import="com.peisia.dto.Dto"%>
...
<!--글읽기 :  http://localhost:8080/spring/guest/read?bno=1 -->
<!-- jstl 로 처리하면 더 짧게 가능 -->
...
<c:set var="totalPage" value="${pageCount/PAGE_LINK_AMOUNT}" />
<c:set var="totalBlock" value="${totalPage/BLOCK_LINK_AMOUNT}" />
<c:forEach var="i" begin="${startBlock }" end="${endPage}">
    <c:choose>
        <c:when test="${i eq currentPage_for}">
            ${i}
        </c:when>
        <c:otherwise>       
