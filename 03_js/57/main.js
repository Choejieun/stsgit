...
while (true) {
	...
	dw(orc.name + "는 " + "<a class=damage>" + elfdamage + "</a>" + "의 피해를 입었습니다.");
	br(); br();
	orc.info(); elf.info();
	hr();
	"<a class = bule>"
	if (elf.hp < 1) {
		...
		br();
		br();
		dw("획득한 경험치 : " + expup);// 획득한 경험치 출력
		br(); 
		dw("획득한 골드 : " + goldup); // 획득한 경험치 출력
		br();
		br();
		break;
	}
	...
	dw(elf.name + "는 <a class=damage>" + orcdamage + "</a>" + "의 피해를 입었습니다.");
	br(); br();
	orc.info(); elf.info();
	hr();
